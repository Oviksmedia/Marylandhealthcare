...
- **Phase 17:** Encounter Note Separation & Textarea Spacebar Bug Resolution — Separated doctor clinical notes from patient reasons by introducing a dedicated database column `doctor_notes` (SQL migration `018_add_doctor_notes_to_appointments.sql`). Resolved the keyboard typing bug where the space bar was intercepted/erased by decoupling the live textarea binding from the dynamic, trimming `stripSystemMetadata()` call and shifting to a dedicated `notesText` React state, which only runs trimming on initialization and final submission.
- **Phase 18:** Secure Existing Email Validation & Duplicate Registration Prevention — Resolved unauthenticated RLS limits blocking the client-side email-exists check by introducing the `checkEmailExists` server action. Refactored the registration pipeline `registerPatientUser` to fail hard with a clean, user-friendly error when attempting to sign up a duplicate patient email, stopping duplicate welcome emails and enforcing login.
